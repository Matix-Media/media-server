...
import { Like, SelectQueryBuilder, SimpleConsoleLogger } from "typeorm";
...
export default function (fastify: FastifyInstanceType, options: RegisterOptions, done: Function) {
    fastify.get<{ Params: { id: string } }>("/image/:id", async (req, res) => {
        try {
    ...
    fastify.get<{ Params: { id: string } }>("/stream/hls/:id", async (req, res) => {
    ...
    fastify.get<{ Params: { id: string } }>("/show/episode/:id", async (req, res) => {
        const episode = await Episode.findOne({ where: { id: req.params.id }, relations: { season: { show: { watchable: true } }, stream: true } });
        if (!episode) throw new NotFound();

        return res.send(episode);
    });

    fastify.get<{ Params: { id: string } }>("/show/episode/:id/next", async (req, res) => {
    ...
    fastify.get<{ Params: { id: string } }>("/movie/:id", async (req, res) => {
    ...
    fastify.get<{ Params: { id: string } }>("/watchable/:id/trailer", async (req, res) => {
